defmodule Slidething.Validator.LayoutTest do
  ...
  describe "zero_area" do
    test "zero width raises error" do
      issues =
        run_checks([
          %{"element_id" => "e1", "x" => 0.1, "y" => 0.1, "width" => 0.0, "height" => 0.4}
        ])

      assert Enum.any?(
               issues,
               &(&1.rule == "zero_area" and &1.severity == :error and &1.target_id == "e1")
             )
    end
    ...
    test "negative height raises error" do
      issues =
        run_checks([
          %{"element_id" => "e1", "x" => 0.1, "y" => 0.1, "width" => 0.3, "height" => -0.1}
        ])
    ...
    test "positive dimensions pass" do
      issues =
        run_checks([
          %{"element_id" => "e1", "x" => 0.1, "y" => 0.1, "width" => 0.3, "height" => 0.4}
        ])
  ...
  describe "overflow" do
    test "element entirely in bounds is fine" do
      issues =
        run_checks([
          %{"element_id" => "e1", "x" => 0.0, "y" => 0.0, "width" => 1.0, "height" => 1.0}
        ])
    ...
    test "right edge overflow" do
      issues =
        run_checks([
          %{"element_id" => "e1", "x" => 0.8, "y" => 0.0, "width" => 0.3, "height" => 0.5}
        ])

      assert Enum.any?(
               issues,
               &(&1.rule == "overflow" and &1.severity == :error and &1.target_id == "e1")
             )
    end
    ...
    test "bottom edge overflow" do
      issues =
        run_checks([
          %{"element_id" => "e1", "x" => 0.0, "y" => 0.8, "width" => 0.5, "height" => 0.3}
        ])
    ...
    test "negative x overflow" do
      issues =
        run_checks([
          %{"element_id" => "e1", "x" => -0.05, "y" => 0.0, "width" => 0.5, "height" => 0.5}
        ])
  ...
  describe "safe_margin" do
    test "element inside safe margin triggers warning" do
      ...
      issues =
        run_checks([
          %{"element_id" => "e1", "x" => 0.01, "y" => 0.1, "width" => 0.5, "height" => 0.5}
        ])

      assert Enum.any?(
               issues,
               &(&1.rule == "safe_margin" and &1.severity == :warning and &1.target_id == "e1")
             )
    end
    ...
    test "element outside safe margin is fine" do
      ...
      issues =
        run_checks([
          %{"element_id" => "e1", "x" => 0.05, "y" => 0.05, "width" => 0.5, "height" => 0.5}
        ])
