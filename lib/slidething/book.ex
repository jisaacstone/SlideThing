defmodule Slidething.Book do
  ...
  def create(title, metadata \\ %{}) do
    ...
    query(
      "INSERT INTO books (id, title, metadata, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
      attrs
    )
  ...
  def create_pages(book_id, pages) when is_list(pages) do
    ids =
      Enum.map(pages, fn %{position: pos} = entry ->
        ...
        query(
          "INSERT INTO pages (id, book_id, position, metadata, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
          [page_id, book_id, pos, meta, now, now]
        )
  ...
  """
  def get(book_id) do
    case query("SELECT id, title, metadata, created_at, updated_at FROM books WHERE id = ?", [
           book_id
         ]) do
      %{num_rows: 0} ->
        {:error, :not_found}
      ...
      %{rows: [[id, title, meta_json, created, updated]]} ->
        pages =
          query("SELECT id, position, metadata FROM pages WHERE book_id = ? ORDER BY position", [
            book_id
          ])

        page_list =
          for [pid, pos, pmeta] <- pages.rows,
              do: %{id: pid, position: pos, metadata: parse_json(pmeta)}
  ...
  def get_outline(book_id) do
    ...
    pages =
      for [pid, pos, meta] <- result.rows,
          do: %{id: pid, position: pos, metadata: parse_json(meta)}
  ...
  def get_page(page_id) do
    case query(
           "SELECT id, book_id, position, metadata, created_at, updated_at FROM pages WHERE id = ?",
           [page_id]
         ) do
      %{num_rows: 0} ->
        {:error, :not_found}
  ...
  def add_format(book_id, format_id) do
    query("INSERT OR IGNORE INTO book_formats (book_id, format_id) VALUES (?, ?)", [
      book_id,
      format_id
    ])
  ...
  def delete(book_id) do
    ...
    for [pid] <- pages.rows do
      query(
        "DELETE FROM element_versions WHERE element_id IN (SELECT id FROM elements WHERE page_id = ?)",
        [pid]
      )
    ...
    query(
      "DELETE FROM agent_messages WHERE agent_run_id IN (SELECT id FROM agent_runs WHERE prompt_id IN (SELECT id FROM prompts WHERE book_id = ?))",
      [book_id]
    )

    query(
      "DELETE FROM agent_runs WHERE prompt_id IN (SELECT id FROM prompts WHERE book_id = ?)",
      [book_id]
    )
